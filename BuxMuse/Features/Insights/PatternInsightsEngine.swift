...
public final class PatternInsightsEngine {
    ...
    public func generateInsights(transactions: [Transaction]) -> [FinancialInsight] {
        ...
        // 1. Weekend vs Weekday Surge Detection
        ...
        if avgWeekend > avgWeekday * 1.50 && avgWeekday > 0 {
            insights.append(FinancialInsight(
                ...
                fullExplanation: "Your weekend spending averages £\(avgWeekend) per day, which is \(Int((Double(truncating: (avgWeekend / avgWeekday) as NSDecimalNumber) - 1.0) * 100))% higher than your weekday average of £\(avgWeekday).",
                severity: .medium,
                category: .pattern,
                systemIcon: "calendar.badge.exclamationmark",
                accentColorName: "orange",
                suggestedActions: [
                    "Establish a concrete 'Weekend Budget cap' of £\(avgWeekday * 1.2).",
                    "Plan free or low-cost activities for Saturday afternoons."
                ],
                impactMonthly: (avgWeekend - avgWeekday) * 8,
                dataBehind: "Weekend Average: £\(avgWeekend). Weekday Average: £\(avgWeekday)."
            ))
        }
        
        // 2. Nighttime Transport Spike (After 8pm)
        ...
        if nighttimeTransportTxs.count >= 2 {
            ...
            insights.append(FinancialInsight(
                ...
                fullExplanation: "You spent £\(nighttimeSum) across \(nighttimeTransportTxs.count) late-night rides this month. Fare spikes and premium options contribute to this nocturnal splurge.",
                severity: .low,
                category: .pattern,
                ...
                dataBehind: "Nighttime Transport Spend: £\(nighttimeSum). Count: \(nighttimeTransportTxs.count)."
            ))
        }
