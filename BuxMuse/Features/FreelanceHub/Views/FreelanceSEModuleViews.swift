...
struct FreelanceIncomeTaxCalculatorView: View {
    @Environment(\.colorScheme) private var colorScheme
    @EnvironmentObject private var themeManager: ThemeManager
    @EnvironmentObject private var freelanceBrain: FreelanceBrain

    var body: some View {
        let snapshot = freelanceBrain.incomeTaxDisplay
...
struct FreelanceQuarterlyTaxView: View {
    @Environment(\.colorScheme) private var colorScheme
    @EnvironmentObject private var themeManager: ThemeManager
    @EnvironmentObject private var freelanceBrain: FreelanceBrain

    var body: some View {
        let snapshot = freelanceBrain.quarterlyDisplay
...
struct FreelanceComplianceAssistantView: View {
    @Environment(\.colorScheme) private var colorScheme
    @EnvironmentObject private var themeManager: ThemeManager
    @EnvironmentObject private var freelanceBrain: FreelanceBrain

    var body: some View {
        let snapshot = freelanceBrain.complianceDisplay
        ...
        .navigationTitle("Compliance Assistant")
        .navigationBarTitleDisplayMode(.inline)
    }
...
struct FreelanceDashboardWidget: View {
    @Environment(\.colorScheme) private var colorScheme
    @EnvironmentObject private var themeManager: ThemeManager
    ...
    private var display: SelfEmployedDashboardDisplay {
        freelanceBrain.selfEmployedDashboardDisplay
    }

    var body: some View {
        if display.hasData {
            Button {
                navigationCoordinator.selectedTab = .freelance
            } label: {
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        Label("Self-Employed", systemImage: "briefcase.fill")
                            .font(.system(size: 11, weight: .bold))
                            .foregroundColor(themeManager.current.accentColor)
                        Spacer()
