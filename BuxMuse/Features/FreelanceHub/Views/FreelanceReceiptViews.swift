...
struct FreelanceReceiptsListView: View {
            ...
            if store.receipts.isEmpty {
                ...
                List {
                    ForEach(store.receipts) { receipt in
                        NavigationLink(destination: FreelanceReceiptDetailView(receipt: receipt).environmentObject(themeManager).environmentObject(appSettingsManager)) {
                            HStack {
...
struct FreelanceReceiptScannerView: View {
    ...
    private func processCapturedImage(_ img: UIImage) {
        ...
        FreelanceReceiptEngine.parseReceipt(image: img) { result in
...
struct FreelanceReceiptDetailView: View {
    @Environment(\.colorScheme) private var colorScheme
    @EnvironmentObject private var themeManager: ThemeManager
    @EnvironmentObject private var appSettingsManager: AppSettingsManager
    ...
    private func infoRow(label: String, value: String, isAmount: Bool = false) -> some View {
        HStack {
            ...
            Text(value)
                ...
                .foregroundColor(isAmount ? .red : (colorScheme == .dark ? .white : .black))
        }
    }
